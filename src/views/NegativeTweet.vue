          ...
          <tbody>
            <tr v-for="data in filteredItems" :key="data.id">
              ...
              <td class="align-middle text-center text-sm">
                <span class="badge badge-sm bg-gradient-error">{{data.classification_result === 1 ? "positive" : "negative"}}</span>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    watch: {
		...
		methods: {
			fetchData: function(page){
				this.loading = true;
				let url = "https://be-sentiment-app-ta.herokuapp.com/negative-tweet/all?hashtag=vaksin"+"&page="+page;
				axios
					.get(url)
					.then(
						function (response) {
